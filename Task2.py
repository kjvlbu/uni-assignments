...
if __name__ == '__main__':
    ...
    print(f"Total Runners: {len(runner_list)}")
    print(f"Average Time: {convert(mean(time_list))}")
    print(f"Fastest Time: {convert(min(time_list))}")
    print(f"Slowest Time: {convert(max(time_list))}")
    print(f"Best Time Here: Runner #{runner_list[time_list.index(min(time_list))]}")
